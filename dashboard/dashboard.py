...
def get_column_metadata(
    column: str,
) -> Tuple[str, str, Optional[MetricCategory], Optional[str], bool]:
...
def create_sidebar_filters(df: pd.DataFrame):
    ...
    with st.sidebar:
        # Type filter
        st.markdown(
            "<h2 style='color: #2c3e50; margin-bottom: 0.5rem;'>Type</h2>",
        ...
        selected_types = (
            [cat for cat in MetricCategory if cat.value.title() == selected_type]
            if selected_type != "All"
            else list(MetricCategory)
        )

        # Source filter
        st.markdown(
            "<h2 style='color: #2c3e50; margin-bottom: 0.5rem;'>Source</h2>",
            unsafe_allow_html=True,
        )
        available_sources = set(
            col.split("__")[0].title() for col in df.columns if "__" in col
        )
        # Use custom order for sources, filtering out any that aren't available
        ordered_sources = [
            source for source in SOURCE_ORDER if source in available_sources
        ]
        ...
        selected_sources = (
            [selected_source.lower()]
            if selected_source != "All"
            else [source.lower() for source in ordered_sources]
        )

        # Time range filter
        st.markdown(
            "<h2 style='color: #2c3e50; margin-bottom: 0.5rem;'>Time Range</h2>",
            unsafe_allow_html=True,
        )
        ...
        # Display options
        st.markdown(
            "<h2 style='color: #2c3e50; margin-bottom: 0.5rem;'>Display Options</h2>",
            unsafe_allow_html=True,
        )
        st.toggle(
            "Show Daily Traces",
...
def filter_columns_by_type_and_source(
    df: pd.DataFrame, selected_types: List[MetricCategory], selected_sources: List[str]
) -> List[str]:
    ...
    for col in df.columns:
        ...
        type_match = not selected_types or (
            metadata and metadata.category in selected_types
        )
        source_match = not selected_sources or any(
            col.startswith(source) for source in selected_sources
        )
...
def calculate_rolling_averages_with_buffer(
    df: pd.DataFrame, columns_to_plot: List[str], time_range: str, buffer_days: int = 60
) -> pd.DataFrame:
    """Calculate rolling averages with a buffer of extra days to ensure proper averaging at the start."""
    # First, determine the date range we want to display
    end_date = df["date"].max()
    if time_range == "All Time":
    ...
    # Add buffer days to the start date to get more data for rolling averages
    buffer_start_date = start_date - timedelta(days=buffer_days)

    # Filter to include buffer data
    buffer_mask = (df["date"] >= buffer_start_date) & (df["date"] <= end_date)
    df_with_buffer = df.loc[buffer_mask].copy()

    # Calculate rolling averages on the buffered data
...
def create_plot(
    ...
    with st.container(border=True):
        ...
        if sum_weekly:
            # For weekly metrics, use buffered data to ensure proper rolling averages
            plot_data = buffered_df if buffered_df is not None else plot_df
            # Pass the display range to filter the final plot
            display_start = plot_df["date"].min()
            display_end = plot_df["date"].max()
            fig = create_weekly_line_plot(
                plot_data, column, category_color, display_start, display_end
            )
        else:
...
def create_weekly_line_plot(
    ...
    rolling_col = f"{column}_rolling"
    plot_df[rolling_col] = (
        plot_df[column].rolling(window=4, min_periods=1, center=False).mean()
    )
    ...
    plot_df.loc[:, "hover_text"] = plot_df.apply(
        ...
        else f"4-week Avg: {row[rolling_col]:.1f}",
        axis=1,
    )
    ...
    try:
        ...
        plot_data = pd.melt(
            ...
            value_vars=[column, f"{column}_rolling"]
            if st.session_state.show_daily_traces
            else [f"{column}_rolling"],
            var_name="variable",
            value_name="value",
        )
        ...
        fig.update_traces(
            line_shape="spline", mode="lines", selector=dict(name=f"{column}_rolling")
        )

        # Add smoothing only for non-Scattergl traces
        for trace in fig.data:
            if trace.name == f"{column}_rolling" and not isinstance(
                trace, go.Scattergl
            ):
        ...
        # Filter the plot to the display range if specified
...
def create_daily_line_plot(df: pd.DataFrame, column: str, color: str) -> Any:
    ...
    try:
        ...
        # Update traces with smoothing for all trace types
        fig.update_traces(
            line_shape="spline", mode="lines", selector=dict(name=rolling_col)
        )
...
def prepare_display_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cols = ["day", "date"] + [
        col for col in display_df.columns if col not in ["day", "date"]
    ]
    ...
    for col in display_df.columns:
        if col == "day":
            ...
            column_names[col] = (
                f"{source.title()} - {pretty_name} ({unit})"
                if unit
                else f"{source.title()} - {pretty_name}"
            )
    ...
    with st.form("manual_data_entry"):
        ...
        with col3:
            ...
            lift = st.selectbox(
                ...
                index=0
                if st.session_state.manual_form_state["lift"] is None
                else (1 if st.session_state.manual_form_state["lift"] else 2),
...
def get_category_color(
    ...
    if is_dual_axis:
        return (
            "#e74c3c" if is_second_metric else "#3498db"
        )  # Red for second metric, blue for first
    if category is None:
...
def create_dual_axis_plot(
    df: pd.DataFrame, metric1: str, metric2: str, buffered_df: pd.DataFrame = None
) -> PlotlyFigure:
    ...
    # Use buffered data for weekly metrics if available
    if buffered_df is not None:
        buffered_plot_df = buffered_df.copy()
    else:
    ...
    def add_metric_traces(
        metric: str, pretty_name: str, sum_weekly: bool, is_secondary: bool = False
    ) -> None:
        ...
        if sum_weekly:
            ...
            weekly_df[rolling_col] = (
                weekly_df[metric].rolling(window=4, min_periods=1, center=False).mean()
            )

            # Filter to display range after calculating rolling averages
            plot_start = plot_df["date"].min()
            plot_end = plot_df["date"].max()
            weekly_df = weekly_df[
                (weekly_df["date"] >= plot_start) & (weekly_df["date"] <= plot_end)
            ].copy()

            # Create hover text for the rolling average
            weekly_df.loc[:, "hover_text"] = weekly_df.apply(
                lambda row: f"Weekly Total: {row[metric]:.1f}<br>4-week Avg: {row[rolling_col]:.1f}"
                if st.session_state.show_daily_traces
                else f"4-week Avg: {row[rolling_col]:.1f}",
                axis=1,
            )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    line=dict(color=color, width=2.5, smoothing=1.3, shape="spline"),
                    yaxis="y2" if is_secondary else "y",
            ...
            # For non-weekly metrics, use daily data with pre-calculated smoothing
            smoothing_window = get_smoothing_window(
                plot_df["date"].min(), plot_df["date"].max()
            )

            # Create hover text for the rolling average
            plot_df.loc[:, "hover_text"] = plot_df.apply(
                lambda row: f"Daily: {row[metric]:.1f}<br>{smoothing_window}-day Avg: {row[rolling_col]:.1f}"
                if st.session_state.show_daily_traces
                else f"{smoothing_window}-day Avg: {row[rolling_col]:.1f}",
                axis=1,
            )
    ...
    fig.update_layout(
        ...
        height=400,
        yaxis=dict(
            title=dict(text=pretty_name1, font=dict(size=12, color=LEFT_COLOR)),
            side="left",
            showgrid=True,
        ...
        yaxis2=dict(
            title=dict(text=pretty_name2, font=dict(size=12, color=RIGHT_COLOR)),
            side="right",
            showgrid=False,
            overlaying="y",
...
def create_compare_mode_section(
    df: pd.DataFrame, available_metrics: List[str], buffered_df: pd.DataFrame = None
):
    ...
    with st.container():
        ...
        for metric in available_metrics:
            ...
            source_index = next(
                (i for i, s in enumerate(SOURCE_ORDER) if s.lower() == source.lower()),
                len(SOURCE_ORDER),
            )
        ...
        with col1:
            metric1 = st.selectbox(
                ...
                format_func=lambda x: "Select a metric..."
                if x is None
                else next((opt[1] for opt in metric_options if opt[0] == x), x),
                index=0
                if st.session_state.compare_metric_1 is None
                else [opt[0] for opt in metric_options].index(
                    st.session_state.compare_metric_1
                )
                + 1,
        ...
        with col2:
            metric2 = st.selectbox(
                ...
                index=0
                if st.session_state.compare_metric_2 is None
                else [opt[0] for opt in metric_options].index(
                    st.session_state.compare_metric_2
                )
                + 1,
        ...
        if metric1 and metric2:
            fig = create_dual_axis_plot(df, metric1, metric2, buffered_df)
            st.plotly_chart(
...
def main():
    ...
    # Get all available columns first (before filtering)
    all_columns = filter_columns_by_type_and_source(
        df, selected_types, selected_sources
    )

    # Get columns to plot from the full dataset
    columns_to_plot = [
        col for col in all_columns if col != "date" and not df[col].isna().all()
    ]
    ...
    # Calculate rolling averages with buffer to ensure proper averaging at the start
    buffered_df = calculate_rolling_averages_with_buffer(
        df, columns_to_plot, selected_time_range
    )

    # Filter to just the desired date range for display
    end_date = df["date"].max()
    if selected_time_range == "All Time":
        start_date = df["date"].min()
    else:
        days = TIME_RANGES[selected_time_range]
        start_date = end_date - timedelta(days=days)

    display_mask = (buffered_df["date"] >= start_date) & (
        buffered_df["date"] <= end_date
    )
    filtered_df = buffered_df.loc[display_mask].copy()

    # Filter to just the columns we need
    filtered_df = filtered_df[
        all_columns + [f"{col}_rolling" for col in columns_to_plot]
    ]
    ...
    for row in range(num_rows):
        ...
        for col_idx in range(num_columns):
            ...
            if metric_idx < len(columns_to_plot):
                ...
                with cols[col_idx]:
                    source, pretty_name, category, unit, sum_weekly = (
                        get_column_metadata(column)
                    )

                    if selected_time_range == "All Time":
                        metric_data = filtered_df[["date", column]].dropna(
                            subset=[column]
                        )
                        if not metric_data.empty:
                            metric_start_date = metric_data["date"].min()
                            metric_mask = (filtered_df["date"] >= metric_start_date) & (
    ...
    # Remove rolling average columns from display table
    display_columns = [
        col for col in filtered_df.columns if not col.endswith("_rolling")
    ]
    display_df = prepare_display_table(filtered_df[display_columns])
    st.dataframe(display_df, use_container_width=True, hide_index=True)

    # Add compare mode section
    create_compare_mode_section(filtered_df, columns_to_plot, buffered_df)
