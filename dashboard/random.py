    ...
    for i, date in enumerate(dates):
        ...
        if True:  # Always available
            ...
            sleep_duration = np.clip(
                random.gauss(7.5 + (0.5 if is_weekend else 0), 0.8), 6.0, 10.0
            )
            ...
            steps = int(
                np.clip(
                    random.gauss(
                        8000 + 4000 * base_fitness * weekly_load * activity_modifier,
                        2000,
                    ),
                    2000,
                    25000,
            ...
            sleep_hr = int(np.clip(base_rhr * random.uniform(0.85, 1.05), 35, 60))
        ...
        if random.random() > 0.05:  # 95% tracking compliance
            ...
            protein = np.clip(random.gauss(100 + weekly_load * 50, 20), 60, 200)

            carbs = np.clip(random.gauss(200 + weekly_load * 80, 40), 100, 400)

            fat = np.clip(random.gauss(80 + weekly_load * 20, 15), 40, 150)
            ...
            saturated_fat = np.clip(
                ...
                10,
                60,  # Reasonable range for saturated fat
            )
        ...
        weekly_runs_so_far = sum(
            1
            for j in range(max(0, i - 6), i)
            if j < len(daily_data)
            and daily_data[j].garmin
            and daily_data[j].garmin.total_distance_km
        )

        run_probability = (
            0.8
            if (
                weekly_run_km < target_weekly_km * 0.8
                and weekly_runs_so_far < runs_per_week
            )
            else 0.1
        )
        ...
        if random.random() < run_probability:
            ...
            garmin_data = GarminData(
                ...
                vo2_max=round(
                    current_vo2 + random.gauss(0, 0.5), 1
            ...
            if random.random() > 0.1:  # Sometimes Garmin data even without runs
                total_steps = int(np.clip(random.gauss(8000, 2000), 3000, 15000))
        ...
        if (
            garmin_data
            and garmin_data.total_distance_km
            and garmin_data.total_distance_km > 10
        ):
