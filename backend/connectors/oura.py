...
class OuraConnector(Connector[OuraData]):
    ...
    def _calculate_weighted_average(self, items: list[float | None], interval: int) -> float | None:
    ...
    def get_data(
        ...
        # Get sleep data
        sleep_data = cast(list[OuraSleepEntry], self.client.get_daily_sleep(start_date=start_str, end_date=end_str))
        sleep_periods = cast(list[OuraSleepPeriod], self.client.get_sleep_periods(start_date=start_str, end_date=end_str))
        
        # Get readiness data
        readiness_data = cast(list[OuraReadinessEntry], self.client.get_daily_readiness(start_date=start_str, end_date=end_str))
        
        # Get activity data
        activity_data = cast(list[OuraActivityEntry], self.client.get_daily_activity(start_date=start_str, end_date=end_str))
        ...
        for sleep_entry in sleep_data:
            ...
            readiness_entry = next((r for r in readiness_data if r["day"] == sleep_entry["day"]), None)
            activity_entry = next((a for a in activity_data if a["day"] == sleep_entry["day"]), None)
            ...
            for period in periods_by_day.get(sleep_entry["day"], []):
                ...
                if heart_rate_data is not None and isinstance(heart_rate_data, dict):
                    ...
                    if hr_items is not None and hr_interval is not None and isinstance(hr_items, list):
                        ...
                        if valid_hrs:
                            period_lowest = int(min(valid_hrs))  # Convert to int since that's what the model expects
                ...
                if hrv_data is not None and isinstance(hrv_data, dict):
                    ...
                    if hrv_items is not None and hrv_interval is not None and isinstance(hrv_items, list):
                        hrv_items = cast(list[float | None], hrv_items)
                        hrv_avg = self._calculate_weighted_average(hrv_items, hrv_interval)
                        if hrv_avg is not None:
                            total_hrv_weighted += hrv_avg * duration
            
            # Calculate final averages
            sleep_heart_rate = total_hr_weighted / total_weight if total_weight > 0 else None
            ...
            oura_data = OuraData(
                ...
                sleep_duration_hours=total_duration / 3600 if total_duration > 0 else None,
                readiness_score=readiness_entry.get("score") if readiness_entry else None,
